...
{
	"version": 1,
	"profile": ["anthropic", "anthropic-updates", "npm", "github"],
	"mode": "enforce",
...
**egress とは別の話ですが、同じ実機で測ったので併記します。** エージェントに egress の前提を渡すとき、どのファイルが実際に読まれるかという問題です。**2026-08-03、Claude Code v2.1.221 で測定しました。**
...
| `CLAUDE.md` からの `@` インポート（**プロジェクト外の絶対パス**、`@/etc/...` や `@/home/...`） | **展開されない** |

**測り方:** カナリア文字列を書いたファイルを各所に置き、空のディレクトリで `claude -p` を実行して、その文字列を出力できるかを見ました。展開されなければ `UNKNOWN` が返ります。

**帰結が 2 つあります。**

* **`AGENTS.md` に書いても Claude Code には届きません。** 他のエージェント（Codex など）向けには有効です
* **プロジェクトの外に置いたファイルを `@` で常時読み込ませることはできません。** 常時読ませたいものは `CLAUDE.md` に直接書いてください（[`../README.md`](../README.md) の「常時読ませるもの」）。**必要になった時点で読ませるだけなら制約はありません** — エージェントは任意のパスのファイルを読めます

**バージョンが上がったら測り直してください。** 手順は上のとおりで、5 分で終わります。
