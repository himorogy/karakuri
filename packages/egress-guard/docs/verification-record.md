...
## 3. 検証で見つかった欠陥と、回帰させないこと

まず実装の欠陥を表にまとめ、そこから読み取れるパターン、回帰させないための落とし穴、テストスイート自身の欠陥をこの順で置きます。

**表に挙げたものはすべてユニットテスト（記録型スタブ）を通過していました。** 各件の「なぜ捕まらなかったか」が、テスト設計の改善点になります。
...
| 7 | `firewall.json` の文字列に制御文字を入れると、レビューで見た値と実際に効く値が食い違う | 2026-08-03<br>（codex によるコードレビュー） | **スタブどころか入力が印字可能文字しか無かった。** `--check-config` のケースはすべて ASCII の印字可能文字で書かれており、「bash が運べない文字」をそもそも一度も食わせていなかった | `NUL in profile` / `NUL in allowDomains` / `NUL in allowCidrs` / `NUL in a field name` / `newline in allowDomains` / 制御文字メッセージの検査 |
| 8 | GitHub meta の値が `validate_cidr` より先に `aggregate` へ渡る。取得が途中で失敗しても成功と区別がつかない | 2026-08-03<br>（codex によるコードレビュー） | **`aggregate` に何が届いたかを見ていなかった。** 観測していたのは戻ってきた側だけで、下流の `validate_cidr` が最終的に弾くため結果は正しく見える。process substitution の中の失敗も終了状態に出ない | `meta ranges are checked before aggregate is handed them` / `a meta response that cannot be read to the end says so`。`aggregate` スタブが受け取った行を `aggregate< ...` として記録するようにした |

**欠陥 7 は I5 違反ではありません。** validator が見る文字列と ipset に入る文字列は同一で、I5 は字義どおり成立しています。壊れているのは「**レビューした値と、実際に効く値が違う**」ことです。`jq -r` は `\u0000` を実 NUL バイトとして出力し、コマンド置換も `read` も NUL を運べないため、PR の diff には `attacker\u0000.example.com` が見えて、実効 allowlist には `attacker.example.com` が入ります。改行・タブも同様に、1 エントリが 2 エントリに割れる／途中で切れるという形で同じ食い違いを作ります。

これが重いのは、**I1 が「人間の関与」を安全弁にしている**からです。エージェントは実効設定を書けないので runtime の攻撃経路ではありません。攻撃経路は人間レビューのすり抜けで、まさに I1 が守っているはずの一点です。性質としては、ワイルドカードを却下した理由（受理するが実現しない）と同族です。

**欠陥 8 の深刻度は低いままです。** 入力元は TLS 越しの GitHub でエージェントからは届かず、集合は追加のみ、最悪でも range が減るだけ（I3 により meta 取得は best-effort）。それでも「検証を通ったものだけが外部コマンドと ipset に届く」という境界は成立していなかったため、直しました。
...
7 と 8 で見えたのはその次の段です。**スタブの作りではなく、入力の作りが甘い。** 7 は印字可能文字しか食わせていなかった、8 は「外部コマンドに何が届いたか」を一度も観測していなかった。**境界のこちら側から見た結果が正しければ、境界の向こう側に何が渡ったかは問わない**という形のテストは、I5 のような「境界」を主張する不変条件を検査できません。
...
* **アドレスファミリで絞ることと、値を検証することを混同しない。** `select(test("^[0-9]+[.]"))` は前者で、`999999999999.1.1.1/32` を通します。`validate_cidr` は `aggregate` の**前**に置いてください。`aggregate` の出力も入力とは別物なので、ipset に入れる前にもう一度通します（欠陥 8）
* **bash が運べない文字は、bash に渡す前に殺す。** `jq -r` は `\u0000` を実 NUL バイトとして出力し、コマンド置換も `read` も NUL を運べません。改行は 1 エントリを 2 つに割り、タブは IFS で切ります。**「検証を通った値がそのまま届く」だけでは足りません。レビューで見た値と届く値が同じである必要があります**（欠陥 7）
* **process substitution の中で失敗を捨てない。** `while read ... < <(cmd1 | cmd2)` は `cmd1` / `cmd2` の終了状態をどこにも出しません。途中で死んでも「取れた分」で正常終了したように見えます。いったん変数に受けて終了状態を見てください（欠陥 8）
* **「N 件許可しました」だけのログを書かない。** 落とした件数が出ないログは、短いリストと完全なリストの区別を消します。best-effort な追加ほど、切り詰めが起きたことを言う必要があります（欠陥 8）
...
```
192 passed, 0 failed, 10 skipped
```
...
### テスト自体の欠陥: `iptables-restore` スタブが、拒否されたテーブルを実効テーブルとして記録していた（修正済み）

**これで 3 度目です。** 上の表の欠陥 2・4（部分一致アサーションと `--` 誤用、常に成功する `ipset test` スタブ）、直前の configuration source に続く、**同じ「テストが通っているが検査していない」形**です。2026-08-03、codex によるコードレビューで指摘されました。

スタブは入力を状態ファイルへコピーしてから失敗判定をしていました。

```bash
cat >"$FW_STATE/filter-v4.$run.$n"
cp "$FW_STATE/filter-v4.$run.$n" "$FW_STATE/filter-v4.$run"   # ← 先にコピー
case ",${FW_FAIL_RESTORE:-}," in *",$n,"*) exit 1 ;; esac
```

実 netfilter はテーブルを丸ごと適用するか一切適用しないかのどちらかで、拒否された restore の後は**直前に受理されたテーブルが実効のまま**です。このスタブでは拒否されたテーブルが実効として残るため、「失敗したら X にフォールバックする」という形のアサーションが、何も確かめずに通ります。

**ただし、指摘に含まれていた「`FW_FAIL_RESTORE=2,3` のケースが空振りしていた」は誤りでした。** 実際の restore は 4 回で、1 が bootstrap、2 が recorder 付きの最終テーブル、3 が recorder 抜きの最終テーブル、**4 が panic テーブル**です。`2,3` は panic を失敗させないので、当該ケースはもともと正しい理由で通っていました（`v4count` が 4 であることを確認しました）。

**本当の穴は、panic restore 自身が失敗する場合を一度も試していなかったことです。** panic テーブルは `emit_panic_filter | iptables-restore 2>/dev/null` と戻り値を見ずに流し込まれるため、これが拒否されても検出されません。そのとき残るのは bootstrap テーブルで、これは DNS 以外を落としているので I2 は成立していますが、**「成立している」ことを示すテストはありませんでした。**

修正は 2 つです。

* スタブの失敗判定を `cp` の**前**へ移し、**受理されたテーブルだけ**が `filter-v4.<tag>` に残るようにした（入力は最後まで読むので上流は SIGPIPE で死なない。生成物の中身を見るアサーションのために `filter-v4.<tag>.<n>` は失敗時も残す）
* `panicfail` ケースを追加した。`FW_FAIL_RESTORE=2,3,4` で exit≠0、実効テーブルは bootstrap のまま（`-p udp --dport 53 -j DROP` があり `sport 22` は無く、`:OUTPUT DROP`）であることを確認する
* あわせて `restorefail` に `v4count` が 4 であることの確認を足した。panic restore が試みられなくなっても、最後のテーブルに sshd の行があれば通ってしまうため

修正前のスタブに対してこの `panicfail` ケースを掛けると 2 件落ちます。検査は生きています。

```
FAIL the effective table is still the bootstrap table (missing: ^-A OUTPUT -p udp --dport 53 -j DROP)
