...
* **受け取った名前を proxy 自身が解決し直し、その結果へ接続する。** クライアントが指定した宛先 IP は使いません。これが無いと、SNI に許可済みドメインを書いて任意の IP へ接続する経路が開きます
* **ECH（Encrypted ClientHello）は拒否する。** SNI が暗号化されると判定材料が消えます。判定できない接続は落とします
* **管理ポート・メトリクスポートを開かない。** sidecar は同一ネットワーク上のコンテナから到達可能です

**着手前に決めること:** 透過型（DNAT）か明示型（`HTTP_PROXY` / `HTTPS_PROXY` と `CONNECT`）か。現時点の傾きは明示型で、着手条件は「proxy 設定に従わないツールの実測」です（[`design.md`](./design.md) §2.22）。
