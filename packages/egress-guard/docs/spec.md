...
現行実現層（L3 の IP 集合）の制限であり、実現層の交代（§10.1）で消えます。**コンテナ内に DNS 連動の allowlist を挟む案は却下しました**（[`design.md`](./design.md) §2.20）。実害の詳細と回避策は [`../README.md`](../README.md) の「コンテナ起動後にセットアップを行う場合」。
