...
* **L7 制御（メソッド別・パス別の制御）はしない。** 将来の proxy 移行でも扱いません（§10.1・[`design.md`](./design.md) §2.23）
...
**この制限は §10.1 の L7 proxy 移行でも解消しません。** 移行後の proxy は TLS を終端しないため、メソッドもパスも見えないままです。解くには MITM proxy が必要ですが、GET 自体が書き出しチャネルである以上メソッドで分ける実益が乏しく、採りません（[`design.md`](./design.md) §2.23）。
...
受容の理由と対策の選択肢は [`design.md`](./design.md) §3.1。実現層の交代（§10.1）で「固定」を強化できますが、**解消はしません。** proxy 側の名前 ACL で未許可名は潰せる一方、許可済みドメインのサブドメインを使うトンネリングは残ります。
...
* 解消: §9.1（SNI / Host ヘッダにドメイン名が乗るため、ワイルドカードが展開なしでマッチできる。スキーマ version 2 で再導入）、**§9.7（名前で判定するため、アドレスが動いても関係がなくなる）**
* **解消しない: §9.2（メソッド別・パス別制御）。** TLS を終端しないため、メソッドもパスも見えません。解くには MITM proxy が要りますが採りません（[`design.md`](./design.md) §2.23）
* 不変条件: I1 — proxy 設定はエージェントのコンテナから不可触の配置（sidecar のイメージへ焼き込むか read-only mount）とし、保存。コンテナ境界が §2.1 の所有権境界を代替する。I2 — proxy が落ちれば外向きは全て落ちるため保存。エージェントから proxy を停止する経路はない。I3 — 縮小後のスクリプトは同一機構のため保存。I4 — 保存。proxy 側の名前ベース ACL と併用すれば「固定」から強化されるが、**許可済みドメインのサブドメインを使うトンネリングは残る**（§9.4 の解消ではない）。I5 — proxy 設定の検証として引き継ぐ。I6 — 保存
* コスト: sidecar の構築・運用。`firewall.json` はドメインリスト中心のまま proxy 設定へ変換する

**中間段は置きません。** コンテナ内に DNS 連動の allowlist を挟む案は却下しました（[`design.md`](./design.md) §2.20）。**この案の要点は、名前を見る層をエージェントの netns の外に出すことです。** 却下した案が抱えていた「特権パーサがエージェントと同居する」問題が、ここでは解決します。SNI-only proxy は ipset を書かないため、そもそも `CAP_NET_ADMIN` を必要としません。

現行の実現層では表現できない実害が 2 件出ています（§9.7、[`known-issues.md`](./known-issues.md) #7）。どちらも L3 に留まる限り回避策がありません。

#### 接続方式は明示型

**明示型（`HTTP_PROXY` / `HTTPS_PROXY` と `CONNECT`）を採ります。透過型（DNAT）は採りません。** 決め手は下の必須要件 3（ECH）です。根拠と、`HTTP_PROXY` に従わないツールの実測結果は [`design.md`](./design.md) §2.23。

#### 実装の必須要件

満たせない proxy 実装は候補から外します。根拠は [`design.md`](./design.md) §2.23。

1. **受け取った名前を proxy 自身が解決し直し、その結果へ接続する。** クライアントが指定した宛先 IP は使いません。これが無いと、許可済みドメインを名乗って任意の IP へ接続する経路が開きます
2. **名前ベースの ACL に IP リテラルを持ち込ませない。** IP リテラル宛は、どのドメイン ACL にもマッチさせず既定 deny に落とします。PTR 逆引きの結果を ACL 照合に使う実装（Squid の `dstdomain` は既定でそうします）では、**敵が自分の IP の rDNS を許可済みドメイン名に設定するだけで ACL を通せます**
