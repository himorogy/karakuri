...
**構造的な解決は §10.1 です。** 名前で判定する層に移せば、アドレスがどれだけ動いても関係がなくなります。**コンテナ内に DNS 連動の allowlist を挟む案は却下しました**（[`design.md`](./design.md) §2.20）。
...
**中間段は置きません。** コンテナ内に DNS 連動の allowlist を挟む案（dnsmasq + `ipset=`）は検討のうえ却下しました。**常駐する `CAP_NET_ADMIN` プロセスが、エージェントの選んだ名前に対する上流応答を処理し続ける構造になる**ためです。[`design.md`](./design.md) §2.20。

**この案の要点は、名前を見る層をコンテナの外に出すことです。** 却下した案が抱えていた「特権パーサがコンテナ内に常駐する」問題が、ここでは逆向きに解決します。

**現行の実現層では表現できない実害が 2 件出ています。** どちらも「名前で判定する層」に移せば消えるもので、L3 に留まる限り回避策がありません。

* **`deb.debian.org`** — TTL 25 秒。`allowDomains` に書いても通らない（§9.7）
* **`*.gallerycdn.vsassets.io`** — VS Code 拡張の配信 CDN。ワイルドカードで書けず、publisher 別の具体名でも IP が回る（[`known-issues.md`](./known-issues.md) #7）

**着手前に決めることが 1 つあります。** ホスト側の proxy をどう配布・起動するかです。devcontainer の外側にコンポーネントが増えるため、`initializeCommand` やホスト側スクリプトのどこに載せるかで、利用側の手順が変わります。
