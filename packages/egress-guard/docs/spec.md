...
* **L7 制御（メソッド別・パス別の制御）はしない。** 将来の proxy 移行でも扱いません（§10.1・[`design.md`](./design.md) §2.22）
...
**この制限は §10.1 の L7 proxy 移行でも解消しません。** 移行後の proxy は TLS を終端しないため、メソッドもパスも見えないままです。解くには MITM proxy が必要ですが、GET 自体が書き出しチャネルである以上メソッドで分ける実益が乏しく、採りません（[`design.md`](./design.md) §2.22）。
...
**sidecar コンテナ**に proxy を置き、本スクリプトは「proxy 宛 + DNS 固定のみ許可」へ縮小する。ドメイン ACL は proxy 側の per-project 設定へ移す。**proxy は TLS を終端せず、ClientHello の SNI と平文 HTTP の Host ヘッダだけで判定する**（以下 SNI-only）。配置と TLS を終端しない判断の根拠は [`design.md`](./design.md) §2.22。

* 解消: §9.1（SNI / Host ヘッダにドメイン名が乗るため、ワイルドカードが展開なしでマッチできる。スキーマ version 2 で再導入）、**§9.7（名前で判定するため、アドレスが動いても関係がなくなる）**
* **解消しない: §9.2（メソッド別・パス別制御）。** TLS を終端しないため、メソッドもパスも見えません。解くには MITM proxy が要りますが採りません（[`design.md`](./design.md) §2.22）
...
**明示型（`HTTP_PROXY` / `HTTPS_PROXY` と `CONNECT`）を採ります。透過型（DNAT）は採りません。** 決め手は下の必須要件 3（ECH）です。根拠と、`HTTP_PROXY` に従わないツールの実測結果は [`design.md`](./design.md) §2.22。

#### 実装の必須要件

満たせない proxy 実装は候補から外します。根拠は [`design.md`](./design.md) §2.22。
...
**実装候補は Squid（明示型 `CONNECT`、`ssl_bump` なし）です。まだ実測していません。** 選定理由とマイナス点は [`design.md`](./design.md) §2.22。
