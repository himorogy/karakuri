...
**本仕様の中核です。** §2 以降の全セクションは、これらの不変条件を成立させる**現行の実現層**（iptables / ipset / 配置規約）の記述です。実装を変更するときは各不変条件が保存されることを示し、将来拡張（§10）は「不変条件を保存したまま実現層を替える・足す提案」として記述します。不変条件を弱める変更は、明示的な仕様変更としてのみ行います（§7-9）。
...
* 限界: スクリプト自体が走らなかった初回起動の「開始前」は Docker 既定の全 ACCEPT（§4.6）。`postStartCommand` の失敗として表面化させることで運用側に倒している。原理的に消すには entrypoint 方式（§10.5）
...
|---|---|---|---|
| I1 | §2.1、§3.1、§6.4 | §2.1・§2.16・§2.17・§2.19 | §11-I1 |
...
SIGKILL では trap が発火しません。それでも「開いたまま」が残らないのは §4.4 の適用方式と §4.2 の順序によるものです。ただし**スクリプトが最初の `iptables-restore` に到達する前に殺された場合**は何も変更されず、直前の状態が残ります。これは「エラーだが全開で起動」ではなく「スクリプトが走らなかった」状態で、`postStartCommand` の失敗としてコンテナ起動側に伝わります（I2 の限界。原理的に消すには §10.5）。
...
| テスト | 期待 | 検証対象 | スキップ条件 |
|---|---|---|---|
| 割り当てられたリゾルバで A レコードが返る | 成功 | I4 | — |
| 外部 DNS（`dig @<probe>`）が応答を返さない | 失敗 | I4 | 全プローブが設定済みリゾルバと一致 |
| 許可先（`api.anthropic.com`）に到達できる | 成功 | I7 | — |
...
9. **不変条件を弱める変更は仕様変更としてのみ行う** — 実装・構成の変更をレビューするときは、I1〜I6 それぞれが保存されているかを確認する。保存されない変更は、この仕様書の不変条件の書き換えを伴う明示的な決定として design.md に記録する
...
* **DNS 応答を検証済みデータとして扱わない**（I5、§4.5）。`allowCidrs` にある検査は、名前解決の結果にも要ります
...
いずれも開発・テスト専用です。**`SUDO_USER` / `SUDO_UID` が設定された状態で引数が渡された場合は拒否します。** sudoers が正しく書かれていれば、そもそも sudo 経由では到達しません（I1 の多重防御）。
...
現行実現層（L3 の IP 集合）の制限です。不変条件そのものの制限ではないため、実現層の交代で解消できます（§10.1・§10.2）。L7 proxy への移行時にスキーマ version 2 で再導入します。根拠は [`design.md`](./design.md) §2.10。
...
受容の理由と対策の選択肢は [`design.md`](./design.md) §3.1。実現層の追加（§10.2）で「固定」を実質的な遮断へ強化できます。
...
allowlist は**起動時に解決した A レコードの集合**です（§4.4）。したがって `allowDomains` が成立するのは、**アドレスがその起動の間ずっと変わらないドメインに限られます。**

2026-08-03 に `deb.debian.org` で実測した例です。
...
**`allowDomains` に書いてあるのに落ちる**という形で現れます。同じ日に `nodejs.org` は問題なく通っており、**CDN の性質によって成否が分かれます。** 「書けば通る」とは限らないという意味で、これは §9.1（ワイルドカード不可）と同じ性質の制限です。
...
**構造的な解決は §10.2 です。** 解決時に ipset へ注入する方式なら、TTL が短いことはむしろ前提になります。

---

## 10. 将来拡張

**将来拡張は「I1〜I6 を保存したまま、実現層を替える・足す提案」として記述します。** 各提案には、(a) 何が解消するか、(b) 各不変条件がどう保存・強化されるか、(c) 増えるコスト、を明記します。不変条件を弱める提案はここには置かず、§1 の不変条件の書き換えを伴う仕様変更として扱います（§7-9）。

### 10.1 L7 proxy 移行（本命）

ホスト側に proxy を置き、本スクリプトは「proxy 宛 + DNS 固定のみ許可」へ縮小する。ドメイン / メソッド ACL は proxy 側の per-project 設定へ移す。

* 解消: §9.1（SNI / Host ヘッダにドメイン名が乗るため、ワイルドカードが展開なしでマッチできる。スキーマ version 2 で再導入）、§9.2（メソッド別・パス別制御）
...
### 10.2 DNS 連動 allowlist（中間段・採否未決）

コンテナ内に dnsmasq 等の小さなリゾルバを置き（設定は `/etc` 配下・root 所有で I1 を保存）、許可ドメインのみ解決して**解決結果を解決時に ipset へ注入**する（`ipset=/domain/set`）。非許可名は NXDOMAIN を返す。

* 解消・強化: §9.1 の部分解消（サブドメインが解決時マッチで本来の意味で動く）。I4 の強化 — 非許可名は再帰自体が起きないため、「経路の固定」が実質的な遮断に近づき、クエリログで試行の可視化もできる（現行 recorder に映らない唯一のシグナル）。加えて、起動時スナップショット方式の構造的弱点（CDN の IP 変動、再解決運用、§5.2 の判定問題）が TTL ベースの解決時注入で消える
* 不変条件: I1 — dnsmasq 設定も root 所有固定パス。I5 — 設定生成は検証済み `firewall.json` の値のみから行う。I2・I3・I6 — 保存
* コスト: 常駐コンポーネントと attack surface が 1 つ増える。L7 proxy より導入は軽い
* 位置づけ: 現行 L3 と §10.1 の中間段。§10.1 を導入する場合は不要になる可能性が高い

### 10.3 entrypoint での起動時 DROP
...
### 10.6 Claude Code sandbox 併用

現状 devcontainer 内 sandbox は namespace 制約で非推奨。devcontainer を外す判断をした場合は本スクリプト自体が不要になり、役割は `.claude/settings.json` に移る。その場合も I1〜I6 は「sandbox 設定の管理」の要件として読み替えられる（設定をエージェント可触の場所に置かない、fail-closed、等）。
