...
**AAAA を持つ許可先への接続が足踏みしないか。** IPv6 は全 DROP で、DROP は ICMP を返しません。一方、許可したホスト名が AAAA を持つと、クライアントは RFC 6724 に従って**先に IPv6 を試します**。
...
**接続はするが毎回遅くなる**、という失敗の仕方があり得ます。

現構成では発生しません。実測で `http://host.docker.internal:15432/` が **0.048 秒**で応答しており、コンテナに IPv6 アドレスが無いため AAAA が試行されないためです。
...
現在 IPv4 の未許可先は `REJECT --reject-with icmp-admin-prohibited` で即座に落としているのに対し、**IPv6 だけが DROP という非対称**になっています。REJECT にすればフォールバックが即座に起こり、遮断の強度は変わりません。

**現時点では実装しません。** 発生していない問題のために、検証済みの IPv6 テーブルを変更して再検証を強いるのは筋が悪いためです。

### あわせて未確認

`curl -6` による遮断の実到達性テスト。global IPv6 アドレスが無いため自己検証でもスキップされます。
