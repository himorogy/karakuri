...
ままであることの両方を拒否します。`.env.keys` は私鍵そのもので、これがあれば暗号化した `.env` は
誰でも復号できます。検出しても報告するのはパス・行番号・鍵名までで、`=` より後は決して出力しません。
CI のログはリポジトリのファイルとは保持期間も配信経路も違うので、そこへ secret を写さないためです。
...
書式は 1 行 1 ディレクティブで、次の例のコメント行を除く先頭 3 行は既定と同じ内容です。
...
# production.env のような名前も対象にするなら足す
pattern (^|/)[^/]*\.env$
