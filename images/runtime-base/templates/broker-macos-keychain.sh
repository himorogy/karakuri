...
# よい。どの broker を標準とするかは未決事項（設計書
# .local/prod-secret-isolation-design.md §11）。これはあくまで macOS 向けの
# 一実装。
#
# --- broker 契約（§4.1） ------------------------------------------------------
...
#   （設計書 R11）。dev container からは直接は呼べない（Docker socket が
...
# entrypoint も空 secret を検出して落ちるが（設計書 §4.6）、broker 側で
# 先に止めておいた方が問題の切り分けが早い（I6）。
if [ -z "$dotenv" ]; then
