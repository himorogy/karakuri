...
#
#   登録・更新は同梱の broker-macos-keychain-set.sh を使う（base64 への畳み
#   込みと格納までを行う。クリップボード不要、値は argv にも載らない）:
#
#     BROKER_KEYCHAIN_SERVICE="<project>-prod-env" broker-macos-keychain-set.sh
#
#   手動でやる場合の同等手順:
#
#   1. dotenv 全文を base64 にしてクリップボードへ。端末でそのまま実行し、
#      KEY=value を行ごとに打ち込んで最後に Ctrl-D（stdin 入力なので
#      シェル履歴に残らず、ファイルにも書かない）:
#
#        base64 | pbcopy
#
#   2. Keychain へ登録。プロンプトが出たら base64 の 1 行を貼り付ける。
#      -T "" は「この項目を無認可で読めるアプリを無しにする」指定。省くと
#      項目を作成したアプリが信頼リストに入り、以後の取り出しが認可
#      プロンプトなしで通ることがある。既存項目の更新は -U を足す:
#
