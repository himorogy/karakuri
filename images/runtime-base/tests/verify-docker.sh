...
# --- compose ファイル生成 --------------------------------------------------------
# host-tools/compose.prod.yaml は変更しない (rev.5 確定後にオーケストレーターが
...
# --- A: 出荷 host-tools/compose.prod.yaml の構造検証 (docker が要る、jq を使う) --
#
# 下の compose-current.yaml 等は全て自前生成物であり、実際に配布する
# host-tools/compose.prod.yaml 自身は一度も検証されていなかった。自前生成物
...
COMPOSE_PROD_YAML="$REPO_ROOT/host-tools/compose.prod.yaml"
...
# --- B: 出荷 host-tools/compose.prod.yaml からの最小派生 (docker が要る) --------
#
...
# compose-current.yaml: host-tools/compose.prod.yaml と同一の tmpfs 記法
...
# docker run に直接 host-tools/compose.prod.yaml と同じオプションを渡す。
...
# (host-tools/compose.prod.yaml) は「root:root で作られ USER node が
...
# entrypoint に焼くのはこのタスクのスコープ外 (host-tools/compose.prod.yaml /
...
EOS
	...
	PROD_COMPOSE_FILE="$SCRATCH/compose-current.yaml" \
		...
		GIT_REF="$TEST_COMMIT" \
		bash "$REPO_ROOT/host-tools/prod-run.sh" true >/dev/null 2>&1 || rc=$?
	[ "$rc" -ne 0 ]
}
...
# =============================================================================
# A19〜A34: 出荷 host-tools/compose.prod.yaml の構造検証 (docker が要る、jq を使う)
#
...
# =============================================================================
# B1〜B4: 出荷 host-tools/compose.prod.yaml からの最小派生の実挙動 (docker が要る)
#
