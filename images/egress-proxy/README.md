...
`@himorogy/egress-guard` の `layer: "l7"`（既定）が使う L7 proxy sidecar の、GHCR で配布する prebuilt image。

```
ghcr.io/himorogy/egress-proxy:1
```

**何のために** — squid の据え方（非 root、`-N`、ACL の置き場所、cache manager の拒否、`dstdomain -n`）はどのプロジェクトでも変える理由が無い部分で、それを prebuilt image に焼き込むことで、利用側に残るのは `firewall.json` から ACL と `mode` を焼く 1 コマンドだけになる。
...
`FROM` の直後から `USER proxy` のままで、`USER root` / `USER proxy` の 2 行を挟む必要はない（`egress-proxy-bake` の書き込み先はあらかじめ `proxy:proxy` 所有にしてある）。
...
`.github/workflows/egress-proxy.yml` が `linux/amd64,linux/arm64` のマルチアーキビルドと GHCR への push を行う。
`runtime-base` / `devcontainer-base` のリリースとは独立で、順序の縛りは無い。
...
タグ名は `egress-proxy-v<MAJOR>.<MINOR>.<PATCH>`。
形式が違うとワークフローが検証で落ちる。
試作は `workflow_dispatch` の `:edge` を使う。
