...
> **2026-08-04 追記（本資料は作成時点の記録）。** B 層の中身は `firewall.json` だけに
> 絞り込んだ。egress-guard を 1 個の塊として B に置いていたが、分解すると
> スクリプト本体（`/usr/local/bin/init-project-firewall.sh`）と sudoers は
> 全プロジェクトで同一・root 所有の場所が必要・npm では届かない領域を触る、という
> A 層の条件を満たす。「base に焼くと許可ドメインの差し替えが base の再ビルドを
> 要求する」という懸念が当たるのは実効設定だけで、その実効設定は `COPY` 元が
> プロジェクトのビルドコンテキストにある以上そもそも base には入れられない。
> 現在の割り当ては [README.md](./README.md) を参照。
